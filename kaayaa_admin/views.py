    ...
    def get(self,request):
        ...
        try:
            if request.user.usertype == 1:
                if int(request.GET.get('is_pending')) == 1:
        ...
        try:
            if request.user.usertype == 1:
                if request.GET.get('vendor_id'):
                    vendor_instance = TblKaayaVendorDetails.objects.filter(id= int(request.GET.get('vendor_id')))
                    if vendor_instance:
            ...
            error_message = str(e)
            return Response({'success': success, 'data': data, 'message': error_message}, status=responseCode)
        return Response({'success':success, 'data':data, 'message':message}, status=responseCode)
